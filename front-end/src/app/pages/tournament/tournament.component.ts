...
getAllGameFromTournament(){
  this.isLoading = true;
  if (this.tournamentName) {
    this.gameSv.getAllGameFromtournament(this.tournamentName).subscribe((games) =>{
    this.games = games
    if (games.length == 1) {
      games.forEach((game) =>{
 ...
 isFinishedGame(game: Game): boolean {
  if ((game.round === 'OCTAVEFINAL' || game.round === 'QUARTERFINAL' || game.round === 'SEMIFINAL' || game.round === 'FINAL') && game.status === "FINISHED") {
    return true;
  } else {
    return false;
  }
...
getRouterLink(matches: number): string {
  switch (matches) {
    case 8:
      return `/tournament/${this.tournamentName}/quarti-finale`;
    case 4:
      return `/tournament/${this.tournamentName}/semi-finale`;
    case 2:
      return `/tournament/${this.tournamentName}/finale`;
    default:
